...
public class Create_Table {
    ...
    public Cursor getChartCompleteData() {
        ...
        String selectQuery = " select tc_fce004, tc_fcd004,tc_fcd005, COALESCE(round((CAST(sum(tc_fce008) as REAL) / (select sum(tc_fce008) total from tc_fce_file where tc_fce002 = (select max(tc_fce002) from tc_fce_file)) ) * 100   ,2),0)  g_count " +
                " from tc_fce_file ,tc_fcd_file " +
